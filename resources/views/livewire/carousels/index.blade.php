    ...
    <div class="p-6">
        <x-ui.table :paginator="$carousels" :view="$view">
            ...
            <x-ui.table.tbody 
                x-data="{
                    dragging: null,
                    items: @js($carousels->sortBy('order')->pluck('id')->values()->toArray()),
                    drop(targetId) {
                        if (this.dragging === null || this.dragging === targetId) return;
                        const from = this.items.indexOf(this.dragging);
                        const to = this.items.indexOf(targetId);
                        this.items.splice(to, 0, this.items.splice(from, 1)[0]);
                        $wire.updateOrder(this.items);
                    },
                    move(id, dir) {
                        const i = this.items.indexOf(id);
                        if (dir === 'up' && i > 0) [this.items[i], this.items[i-1]] = [this.items[i-1], this.items[i]];
                        if (dir === 'down' && i < this.items.length-1) [this.items[i], this.items[i+1]] = [this.items[i+1], this.items[i]];
                        $wire.updateOrder(this.items);
                    }
                }">
                @forelse($carousels->sortBy('order') as $index => $carousel)
                    <x-ui.table.tr 
                        draggable="true"
                        x-on:dragstart="dragging = {{ $carousel->id }}"
                        x-on:dragover.prevent=""
                        x-on:drop="drop({{ $carousel->id }})"
                        x-on:dragend="dragging = null"
                        ::class="dragging === {{ $carousel->id }} ? 'opacity-20' : ''"
                        class="transition-all duration-200">
                        <x-ui.table.td>
                            <div class="cursor-grab active:cursor-grabbing text-zinc-300 hover:text-metronic-primary p-2">
                        ...
                        <x-ui.table.td shrink>
                            <div class="flex items-center justify-end gap-1">
                                <div class="flex flex-col mr-2">
                                    <button x-on:click="move({{ $carousel->id }}, 'up')" 
                                        class="p-0.5 text-zinc-400 hover:text-metronic-primary disabled:opacity-10"
                                        {{ $index === 0 ? 'disabled' : '' }}>
                                        <flux:icon name="chevron-up" variant="mini" class="w-3 h-3" />
                                    </button>
                                    <button x-on:click="move({{ $carousel->id }}, 'down')" 
                                        class="p-0.5 text-zinc-400 hover:text-metronic-primary disabled:opacity-10"
                                        {{ $index === count($carousels) - 1 ? 'disabled' : '' }}>
