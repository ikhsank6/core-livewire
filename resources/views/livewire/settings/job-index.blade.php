<x-slot name="breadcrumbs">
    ...
    <flux:breadcrumbs.item>Failed Jobs</flux:breadcrumbs.item>
</x-slot>

<div>
    <x-ui.card title="Failed Jobs" description="Kelola dan coba ulang background jobs yang gagal dijalankan">
        <x-slot name="headerAction">
            <div class="flex items-center gap-2">
                <flux:button wire:click="reload" variant="ghost" icon="arrow-path" size="sm"
                    class="h-10! px-3! rounded-xl! border border-zinc-200! dark:border-zinc-700! font-semibold! text-sm! shadow-sm!"
                    tooltip="Muat Ulang">
                    Muat Ulang
                </flux:button>
                @if($failedJobs->total() > 0)
                    <flux:button wire:click="retryAll" wire:loading.attr="disabled" wire:target="retryAll"
                        variant="primary" icon="arrow-path" size="sm" class="h-10! rounded-xl!">
                        Coba Ulang Semua
                    </flux:button>
                    <flux:button wire:confirm="Apakah Anda yakin ingin menghapus semua failed jobs?" wire:click="deleteAll"
        ...
        <x-ui.table :paginator="$failedJobs">
                    ...
                    <x-ui.table.tr x-data="{ expanded: false }" class="align-top">
                        ...
                        <x-ui.table.td>
                            <div class="flex flex-col gap-1 max-w-2xl">
                                ...
                                <button @click="expanded = !expanded"
                                    ...
                                    <span x-text="expanded ? 'Sembunyikan Exception' : 'Lihat Full Exception'"></span>
                                </button>
                                <div x-show="expanded" x-cloak
                                    ...
                                    <div class="font-mono text-[11px] leading-relaxed text-zinc-700 dark:text-zinc-400 whitespace-pre-wrap break-all">
                                        {{ $job->exception }}
                                    </div>
                                    <div class="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                                        ...
                                        <pre class="text-[10px] font-mono text-zinc-600 dark:text-zinc-500 overflow-x-auto">{{ json_encode($payload, JSON_PRETTY_PRINT) }}</pre>
                                    </div>
                                </div>
                            </div>
                        </x-ui.table.td>
                        <x-ui.table.td shrink>
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <flux:button wire:confirm="Apakah Anda yakin ingin menghapus failed job ini?"
                ...
                @empty
                    <x-ui.table.tr>
                        <x-ui.table.td colspan="5">
                            <x-ui.empty-state message="Tidak ada failed jobs." />
                        </x-ui.table.td>
                    </x-ui.table.tr>
                @endforelse
            </x-ui.table.tbody>
            ...
            @if($failedJobs->hasPages())
                <x-slot name="footer">
                    <x-ui.pagination :paginator="$failedJobs" perPage="perPage" />
                </x-slot>
            @endif
        </x-ui.table>
    </x-ui.card>
